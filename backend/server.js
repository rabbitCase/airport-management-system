...
app.post('/register', (req,res) => {
    ...
    if (!allowedstaff.mutableArrayObject.allowed.includes(Number(staffid))) {
        return res.json({
            message: "not allowed",
            details: "user is not employed by the airport and/or the employment status is not updated in the database"
        });
    }

    registeredstaff.mutableArrayObject.registered.push(Number(staffid));//add the registered staff to the registered array

    console.log("Updated registered staff: ",registeredstaff.mutableArrayObject.registered);

    allowedstaff.mutableArrayObject.allowed = allowedstaff.mutableArrayObject.allowed.filter(value => value != Number(staffid));//remove this staff from the allowed array

    console.log("Updated allowed staff: ",allowedstaff.mutableArrayObject.allowed);
...
app.post('/login', (req,res) => {
    ...
    if(registeredstaff.mutableArrayObject.registered.includes(Number(req.body.staffid))){
