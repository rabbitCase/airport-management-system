...
//server code
function locationToId(location){//convert location to reference id used by database
...
function airlineToId(id){//convert airline id to airline name referrenced by database
    if(id === 1){
        return "Air India";
    }
    else if(id === 2){
...
button.addEventListener('click', async () => {
    ...
    try {
        ...
        response.forEach(key => {
            ...
            travelPath.textContent = `${idToLocation(key.DepartureAirportID)} to ${idToLocation(key.ArrivalAirportID)}`;
            ...
            header.textContent = "DEPARTURE TIME";
            header.style.fontWeight = '900';
            deptime.appendChild(header);
